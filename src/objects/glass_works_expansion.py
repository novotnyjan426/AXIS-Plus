from objects.base_object import ExpansionObject

SPRITE_FILE = "src/graphics/industries/glass_works_1.png"
CLASS_ID = "AXSB"
CLASS_NAME = "STR_OBJECT_CLASS_BUILDING_MATERIALS"
GROUND = "GROUNDSPRITE_CONCRETE"
SMOKE = "3701 + (animation_frame % 8)"

# Factory with smoke
factory = ExpansionObject(
    id="glass_works_exp_factory",
    numeric_id=427,
    name_string="STR_OBJECT_GLASS_WORKS_FACTORY",
    class_id=CLASS_ID,
    class_name_string=CLASS_NAME,
    sprite_file=SPRITE_FILE,
    size=[1, 1],
    ground_sprite=GROUND,
    height=10,
    animation_length=8,
    animation_speed=3,
    tile_grid=[
        {"x": 0, "y": 0, "buildings": [
            {"coords": [150, 60, 64, 90, -31, -59]},
            {"sprite_expr": SMOKE, "xoffset": 13, "yoffset": 0, "zoffset": 54,
             "xextent": 15, "yextent": 7, "zextent": 7},
        ]},
    ],
)

# Static building
building = ExpansionObject(
    id="glass_works_exp_building",
    numeric_id=428,
    name_string="STR_OBJECT_GLASS_WORKS_BUILDING",
    class_id=CLASS_ID,
    class_name_string=CLASS_NAME,
    sprite_file=SPRITE_FILE,
    sprite_coords=[80, 60, 64, 90, -31, -71],
    ground_sprite=GROUND,
    height=10,
)
